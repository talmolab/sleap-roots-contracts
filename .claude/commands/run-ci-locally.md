...
```
[1/4] Black formatting check...
would reformat src/sleap_roots_contracts/cross_experiment_analysis.py
1 file would be reformatted, 22 files would be left unchanged.
FAILED

Fix: Run 'uv run black src/sleap_roots_contracts tests' to auto-fix formatting
```
...
- **Black fails**: `uv run black src/sleap_roots_contracts tests`
- **Ruff fails**: `uv run ruff check --fix src/sleap_roots_contracts` (then fix remaining manually)
...
- **Lint job step 1**: `uv run black --check src/sleap_roots_contracts tests`
- **Lint job step 2**: `uv run ruff check src/sleap_roots_contracts`
- **Test job**: `uv run pytest --cov=src/sleap_roots_contracts --cov-report=xml --durations=-1 tests/`

CI runs on: ubuntu-latest, windows-latest, macos-14 with Python 3.11.
...
|-------|-------------|------------|--------|
| Black | `uv run black --check src/sleap_roots_contracts tests` | Same | Yes |
| Ruff | `uv run ruff check src/sleap_roots_contracts` | Same | Yes |
