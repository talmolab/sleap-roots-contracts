---
description: Guide through complete release process for sleap-roots-contracts package
---

# Release Process for sleap-roots-contracts

Comprehensive workflow for releasing a new version of the sleap-roots-contracts package to PyPI.
...
uv run ruff check src/sleap_roots_contracts tests

# Full test suite with coverage (dev dependency)
uv run pytest tests/ -x -q --cov=src/sleap_roots_contracts --cov-report=term-missing --cov-branch
```
...
gh release create v$NEW_VERSION \
  --title "sleap-roots-contracts v$NEW_VERSION" \
  --prerelease \
  --notes "$(cat <<EOF
## Installation

\`\`\`bash
pip install sleap-roots-contracts==$NEW_VERSION
\`\`\`
...
**Full Changelog**: https://github.com/talmolab/sleap-roots-contracts/commits/v$NEW_VERSION
...
echo "PyPI: https://pypi.org/project/sleap-roots-contracts/$NEW_VERSION/"
echo "GitHub: https://github.com/talmolab/sleap-roots-contracts/releases/tag/v$NEW_VERSION"
```
...
# Go to: https://pypi.org/manage/project/sleap-roots-contracts/releases/
