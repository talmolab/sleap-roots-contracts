...
```bash
uv run black --check src/sleap_roots_contracts tests && uv run ruff check src/sleap_roots_contracts
```
...
```bash
uv run black src/sleap_roots_contracts tests && uv run ruff check --fix src/sleap_roots_contracts
```
