...
- [x] 1.1 RED: in `tests/test_trait_blob.py`, assert `BlobKind`'s allowed set is exactly
      ...
      `ValidationError`.
- [x] 1.2 GREEN: narrow `BlobKind` to `Literal["predictions_slp"]` in `models.py`.
- [x] 1.3 Reconcile the existing `test_blobref_rejects_unknown_kind` (currently `kind="not_a_kind"`,
...
- [x] 2.1 RED: assert a `RootType` vocabulary of exactly `{"primary","lateral","crown"}`
...
- [x] 2.2 GREEN: add `RootType = Literal["primary","lateral","crown"]`; add required
      `root_type: RootType` to `BlobRef` (no default). Leave `ModelRef.root_type` as `str | None`
      (recorded decision — see `design.md`).
- [x] 2.3 Export `RootType` and `BlobKind` from `src/sleap_roots_contracts/__init__.py`
...
- [x] 3.1 `tests/fixtures/examples.py` — `example_envelope`'s `BlobRef` passes `root_type="primary"`.
- [x] 3.2 `tests/test_envelope.py` — the `BlobRef` in `test_envelope_round_trips` (line ~37) passes
      `root_type="primary"`. (Untracked by the original Impact; it round-trips a full envelope and
      would otherwise raise.)
- [x] 3.3 `tests/test_trait_blob.py` — update the two location-rule tests that omit `root_type`:
...
- [x] 4.1 RED: in `tests/test_schema.py`, assert the rendered `result_envelope` schema's
...
- [x] 5.1 Bump `pyproject.toml` → `version = "0.1.0a2"` (single source of version truth as of #6;
      `__init__.__version__` resolves from metadata, so no code edit). Use `uv version 0.1.0a2`.
- [x] 5.2 Reinstall so the new version is visible, then regenerate **both** schemas:
...
- [x] 5.3 Update `docs/CHANGELOG.md`:
...
- [x] 6.1 Run `/pre-merge-check`: `black --check`, `ruff check`, full `pytest` + coverage, schema
...
- [x] 6.2 `openspec validate update-blobref-root-type --strict` passes.
