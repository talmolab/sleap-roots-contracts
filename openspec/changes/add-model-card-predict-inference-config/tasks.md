...
> grouping"): **Unit A** = the contract change (models, identity, exports, tests, regenerated
> `result_envelope.schema.json`) still at `v0.1.0a2` (groups 1–4); **Unit B** = the release bump —
> `pyproject.toml` version, **both** regenerated schemas, and the CHANGELOG (group 5). Never commit a
> bare RED step. Tests are pure pydantic/schema/idempotency — **no mocks**.
...
- [ ] 1.1 RED: in a new `tests/test_model_card.py`, assert a valid `ModelCard` constructs and retains
      ...
      a `root_type` outside `{primary, lateral, crown}` raises; a card built without `sleap_nn_version`
      has `sleap_nn_version is None`; reassigning a field raises (frozen).
...
- [ ] 1.3 RED: assert `card.to_model_ref("runtime-x")` returns a `ModelRef` with
      ...
      carries the card's `registry_id`, `version`, `root_type`, and `weights_checksum`; and that it
      works when the card's `sleap_nn_version is None`.
...
- [ ] 1.5 RED: assert `ModelCard.model_validate({**selection_metadata, **artifact_identity})`
      succeeds (merged-dict round-trip); and that `model_validate({...6 real fields..., "soybean":
      True, "oks_map": 0.8, "training_config": {...}})` succeeds and ignores the extras.
...
- [ ] 3.1 RED: in `tests/test_identity.py`, assert `compute_idempotency_key(**BASE,
      ...
      and that two distinct output-param dicts yield distinct keys.
...
- [ ] 3.3 RED: in `tests/test_provenance.py`, assert (a) a `Provenance` without the new fields has an
      ...
      **golden** test: a fixed `Provenance` (fixed inputs, no new fields) hashes to an exact hardcoded
      hex digest captured from current behavior — proves byte-identity, not just self-consistency;
      (c) populating `predict_output_params={"peak_threshold": 0.2}` changes the key; (d) two
      provenances differing **only** in `predict_inference_config` (e.g. `device`) with identical
      `predict_output_params` produce the **same** key; (e) two provenances differing **only** in
      `predict_output_params` (`peak_threshold`) produce different keys.
- [ ] 3.4 GREEN: in `Provenance._fill_idempotency_key`, pass
      `predict_output_params=self.predict_output_params` into `compute_idempotency_key`. Capture the
      golden hex for 3.3(b) from the green run.
...
- [ ] 6.2 `openspec validate add-model-card-predict-inference-config --strict` passes.

## 7. Post-merge / post-release (NOT part of this PR)

- [ ] 7.1 After merge: `/openspec:archive add-model-card-predict-inference-config`.
- [ ] 7.2 After the `v0.1.0a3` release is published: `sleap-roots-predict` (branch
